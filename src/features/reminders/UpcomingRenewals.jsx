  ...
  if (upcoming.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-100 bg-white p-4">
        <h3 className="mb-2 text-sm font-semibold text-slate-700">Upcoming Renewals</h3>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                {logoUrl ? (
                  ...
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-slate-100">
                    <span className="text-xs font-bold text-slate-400">{sub.name.charAt(0)}</span>
                  </div>
                )}
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium text-slate-800">{sub.name}</div>
